...
const EditProductScreen = props => {
    ...
    // console.log(prodId)
    const editedProdcut = useSelector(state =>
        // find 不是方法
    ...
    const [formState, dispatchFormState] = useReducer(// 初始化 类似于useState用法
        ...
        formReducer,//传入的方法怎样update
        {
            inputValues: {
                //代替了 设置state的方法
                title: editedProdcut ? editedProdcut.title : '',
                imageUrl: editedProdcut ? editedProdcut.imageUrl : '',
                description: editedProdcut ? editedProdcut.description : '',
                price: '' // 只要不是新建一个商品的情况下 price 始终设置为空
            },
            inputValidities: {
                title: editedProdcut ? true : false,
                imageUrl: editedProdcut ? true : false,
                description: editedProdcut ? true : false,
                price: editedProdcut ? true : false,//price可以在edit已有商品时设置validation为true
            },
    ...
    const submitHandler = useCallback(() => {
        ...
        if (editedProdcut) {
            console.log("have id and formState", formState.inputValues.title)
    ...
    const inputChangeHandler = useCallback((inputIdentifier, inputValue, inputValidity) => {
        ...
        // }
        // setTitle(text);
        console.log("input in edit :",inputValue)
        dispatchFormState({
            // 来源于 初始化的 useReducer 改变state
            type: FORM_INPUT_UPDATE, // dispatch什么样的方法
        ...
        <ScrollView>
            <View style={styles.form}>
                <Input
                    ...
                    initialValue={editedProdcut ? editedProdcut.title : ''}
                    initiallyValid={!!editedProdcut} //两个感叹号表示 if(editProduct) return true else return false
                ...
                <Input
                    ...
                    onInputChange={inputChangeHandler}
                    initialValue={editedProdcut ? editedProdcut.imageUrl : ''}
                    initiallyValid={!!editedProdcut}
                    required
                />
                ...
                {editProdcut ? null :
                    (<Input
                        ...
                        onInputChange={inputChangeHandler}
                        initialValue={editedProdcut ? editedProdcut.price : ''}
                        initiallyValid={!!editedProdcut}
                        required
                        min={0}
                    />)
                ...
                <Input
                    ...
                    onInputChange={inputChangeHandler}
                    initialValue={editedProdcut ? editedProdcut.description : ''}
                    initiallyValid={!!editedProdcut}
                    required
