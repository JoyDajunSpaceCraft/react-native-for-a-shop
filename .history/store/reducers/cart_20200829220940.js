...
export default (state = initialState, action) => {
    switch (action.type) {
        ...
        case ADD_ORDER:
            //clean cart 在一次点击了cart中的结算（Order now）按钮之后 将购物车清空
            return initialState;
        case DELETE_PRODUCT:
            if (!state.items[action.pid]){
                // 判断用户删除的是否存在于cart中 
                return state;//如果不在 就返回当前state
            }
            const updatedItems = {...state.items};
            const itemTotal = state.items[action.pid].sum; // 获得当前要删除的元素的价格 因为sum是累加的所以不存在单独删除一个
